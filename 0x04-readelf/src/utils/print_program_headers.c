...
int print_machine(Elf64_Ehdr *e)
{
	printf("  Machine:                           ");
	if (get_machine(e))
		printf("%s", get_machine(e));
	else
		printf("<unknown>: 0x%x", e->e_machine);
	printf("\n");
	return (0);
}
...
int print_e_version(ehdr_t *e)
{
	printf("  Version:                           0x%lx\n",
		IS_32(e->e64)
			? (unsigned long)e->e32.e_version
			: (unsigned long)e->e64.e_version);
	return (0);
}
 ...
 * Return: 0 on success else exit_status
 */
int print_program_headers(ehdr_t *e)
{
	printf("  Start of program headers:          ");
	if (IS_32(e->e64))
		printf("%u", IS_BE(e->e64)
			? switch_endian4(e->e32.e_phoff)
			: e->e32.e_phoff);
	else
		printf("%lu", IS_BE(e->e64)
			? switch_endian8(e->e64.e_phoff)
			: e->e64.e_phoff);
 ...
 * Return: the string or NULL
 */
char *get_machine2(Elf64_Ehdr *e)
{
	switch (e->e_machine)
	{
